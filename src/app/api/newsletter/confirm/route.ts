  ...
  try {
    ...
    try {
      ...
      if (currentNewsletter) {
        ...
        // Utiliser le HTML existant ou le générer à la volée
        let newsletterHTML = content?.html;

        if (!newsletterHTML && currentNewsletter.newsletterRecipes.length > 0) {
          // Générer le HTML à la volée
          const recipes = currentNewsletter.newsletterRecipes.map(nr => nr.recipe);
          ...
          console.log('📝 HTML généré à la volée pour newsletter:', currentNewsletter.subject);
        }
