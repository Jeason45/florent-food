...
if [ -n "$GOOGLE_APPLICATION_CREDENTIALS_BASE64" ]; then
  ...
  printf '%s' "$GOOGLE_APPLICATION_CREDENTIALS_BASE64" | base64 -d > /app/google-analytics-credentials.json 2>/dev/null || \
  printf '%s' "$GOOGLE_APPLICATION_CREDENTIALS_BASE64" | openssl base64 -d > /app/google-analytics-credentials.json
  ...
  printf '%s' "$GOOGLE_APPLICATION_CREDENTIALS_JSON" > /app/google-analytics-credentials.json
  echo "✅ Credentials file created from JSON"
else
  echo "⚠️  No Google Analytics credentials configured (optional)"
fi
